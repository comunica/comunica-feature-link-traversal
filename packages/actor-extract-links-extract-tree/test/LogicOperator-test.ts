...
describe('LogicOperator', () => {
  ...
  describe('And', () => {
    ...
    describe('apply', () => {
      ...
      it(`given a domain and multiple intervals that are not overlapping and where 
      one is overlapping with the domain then should return a valid new domain`, () => {
        const interval1 = new SolutionInterval([ -26, -25 ]);
        const interval2 = new SolutionInterval([ -24, -23 ]);
        const interval3 = new SolutionInterval([ 1, 3 ]);

        const newDomain = and.apply({
          intervals: [ interval1, interval2, interval3 ],
          domain: aDomain,
        });
        const expectedDomain = SolutionDomain.newWithInitialIntervals(interval3);
        expect(newDomain.getDomain()).toStrictEqual(expectedDomain.getDomain());
      });

      it(`given a domain and multiple intervals that are not overlapping and where 
      multiple is overlapping with the domain then should return a valid new domain`, () => {
        const interval1 = new SolutionInterval([ -26, -25 ]);
        const interval2 = new SolutionInterval([ -24, -23 ]);
        const interval3 = new SolutionInterval([ 1, 2 ]);
        const interval4 = new SolutionInterval([ 3, 4 ]);
        const interval5 = new SolutionInterval([ 5, 6 ]);
        const interval6 = new SolutionInterval([ 22, 24 ]);
        ...
        const expectedDomain = SolutionDomain.newWithInitialIntervals(
          [ interval3, interval4, new SolutionInterval([ 5, 5 ]), interval6 ],
        );
        expect(newDomain.getDomain()).toStrictEqual(expectedDomain.getDomain());
      });
