...
describe('AggregatedStoreMemory', () => {
  ...
  describe('match', () => {
    ...
    it('returns a stream with updating metadata', async() => {
      const it1 = store.match();
      const metadataFirst: MetadataBindings = <MetadataBindings> it1.getProperty('metadata');
      await store.importSource('a', rdfjsSource, ctx);
      const p1 = store.importSource('b', rdfjsSource2, ctx);
      store.end();
      expect(metadataFirst.state.valid).toBe(false);
      expect(it1.getProperty('metadata')).toEqual({
        state: expect.any(MetadataValidationState),
        cardinality: { type: 'estimate', value: 2 },
        variables: [],
      });
      expect(it1.getProperty('lastCount')).toBe(2);
      await p1;
    });
  });
